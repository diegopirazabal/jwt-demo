      ...
      {decodedToken && (
        <div>
          ...
          <div style={{ marginTop: '1rem' }}>
            ...
            <p><small>Nota: La firma solo puede ser verificada en el backend con la clave secreta.</small></p>
          </div>
          
          {decodedToken.payload.exp && (
            <div style={{ 
              marginTop: '1rem',
              backgroundColor: decodedToken.payload.exp * 1000 < Date.now() ? '#f8d7da' : '#d4edda',
              color: decodedToken.payload.exp * 1000 < Date.now() ? '#721c24' : '#155724',
              padding: '0.75rem',
              borderRadius: '4px'
            }}>
              <h4>Información de Expiración</h4>
              <p>
                <strong>Expira el:</strong> {new Date(decodedToken.payload.exp * 1000).toLocaleString()}
              </p>
              <p>
              ...
              {decodedToken.payload.exp * 1000 > Date.now() && (
                <p>
                  <strong>Tiempo restante:</strong> {Math.floor((decodedToken.payload.exp * 1000 - Date.now()) / 1000)} segundos
                </p>
              )}
            </div>
          )}
